...
let gameOver = false;
const board = Array.from({ length: ROWS }, () => Array(COLS).fill(null));//creat 2 d array to creat matrix and call back function that creat another array for each row 
...
{
  gameBoard.innerHTML = '';// recet border to make sure matrix is clear to play
  for (let row = 0; row < ROWS; row++) 
  {
    for (let col = 0; col < COLS; col++) 
    {
      const cell = document.createElement('div');// creat div for each cell 
...
{
  if (gameOver) return;  //GARD PATTERN  If the game is over, ignore clicks.
...
function findEmptyRow(col)
{
  for (let row = ROWS - 1; row >= 0; row--) {
    if (!board[row][col]) return row; //if cell in column is empty return index of row 
  }
  return -1;
}
